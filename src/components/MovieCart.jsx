...
const MovieCart = ({ img, title, movies }) => {
    const { addItem } = useCart();
    return (
        <div className="card border-0 rounded-0 bg-transparent col-6 col-sm-6 col-md-4 col-lg-2" id='movie_card' >
            <div id='card_img'>
                <img src={img} className="card-img-top rounded-0" alt="..." />
                <div id='box_content'>
                    <ul className='list-unstyled m-0'>
                        <li><Link className='card_icon' to="/play"><FaPlay /></Link></li>
                        <li className='my-2'><Link className='card_icon'  onClick={() => (addItem(movies),alertify.success(title + ` Added To Cart`))} to="#"><FaPlus /></Link></li>
